...
<c:if test="${fn:contains(sessionInfo.resourceList, '/teacher/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/teacher/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/teacher/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<title>DEMO管理</title>
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 200,
			height : 200,
			href : '${ctx}/teacher/viewPage?id=' + id
		});
	}
